...
static int __init lime_init_module(void)
{
	...
	method =
	    (sscanf(path, "tcp:%d", &port) ==
	     1) ? LIME_METHOD_TCP : LIME_METHOD_DISK;
	if (digest)
...
static int init()
{
	...
	for (p = iomem_resource.child; p; p = p->sibling) {
		...
		if (mode == LIME_MODE_LIME && write_lime_header(p) < 0) {
			DBG("Error writing header 0x%lx - 0x%lx",
			    (long)p->start, (long)p->end);
			break;
		} else if (mode == LIME_MODE_PADDED
			   && write_padding((size_t)((p->start - 1) - p_last)) <
			   0) {
			DBG("Error writing padding 0x%lx - 0x%lx", (long)p_last,
			    (long)p->start - 1);
			break;
		}
...
static void write_range(struct resource *res)
{
	...
	for (i = res->start; i <= res->end; i += is) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,18)
		is = min((resource_size_t) PAGE_SIZE,
			 (resource_size_t) (res->end - i + 1));
#else
		...
		if (is < PAGE_SIZE) {
			...
			DBG("Padding partial page: vaddr %p size: %lu",
			    (void *)i, (unsigned long)is);
			write_padding(is);
		} else {
...
static ssize_t try_write(void *v, ssize_t is)
{
	...
	ret = RETRY_IF_INTERRUPTED((method ==
				    LIME_METHOD_TCP) ? write_vaddr_tcp(v,
								       is) :
...
static int setup(void)
{
	return (method == LIME_METHOD_TCP) ? setup_tcp() : setup_disk(path,
								      dio);
}
