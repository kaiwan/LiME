...
int setup_tcp()
{
	...
	r = sock_create_kern(&init_net, AF_INET, SOCK_STREAM, IPPROTO_TCP,
			     &control);
	...
	int opt = 1;
	r = kernel_setsockopt(control, SOL_SOCKET, SO_REUSEADDR, (char *)&opt,
			      sizeof(opt));
	if (r < 0) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,2,0)
	r = sock_create_kern(&init_net, PF_INET, SOCK_STREAM, IPPROTO_TCP,
			     &accept);
#elif LINUX_VERSION_CODE > KERNEL_VERSION(2,6,5)
